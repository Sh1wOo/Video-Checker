...
import { UpdateWidget } from "./components/updater/UpdateWidget";
import { UpdateModal } from "./components/updater/UpdateModal";
import AndroidFolderPicker from "./components/AndroidFolderPicker";
...
export default function App() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(".hero-panel", { opacity: 0, y: 18 }, { opacity: 1, y: 0, duration: 0.45 })
        .fromTo(".toolbar > *", { opacity: 0, y: 10 }, { opacity: 1, y: 0, stagger: 0.06, duration: 0.3 }, "-=0.18")
        .fromTo(".progress-section > *", { opacity: 0, y: 8 }, { opacity: 1, y: 0, stagger: 0.05, duration: 0.28 }, "-=0.16")
        .fromTo(".side-column .panel, .tree-panel", { opacity: 0, y: 18 }, { opacity: 1, y: 0, stagger: 0.08, duration: 0.38 }, "-=0.12")
        .fromTo(".stat-card", { opacity: 0, y: 12 }, { opacity: 1, y: 0, stagger: 0.05, duration: 0.28 }, "-=0.2");
    }, appRef);
  ...
  useEffect(() => {
    function handleContextMenu(event: MouseEvent) {
      ...
      if (target.closest(".afp-modal") || target.closest(".afp-context-menu")) return;
  ...
  return (
    <div ref={appRef} className="app-shell">
      <div className="app-container">
              ...
              {page === "recovery" ? (
                ...
                <SettingsPanel
                  ...
                  updater={{
                    ...
                    isOnline: updater.isOnline,
                    updateInfo: updater.updateInfo,
                    error: updater.error,
                    checkForUpdates: updater.checkForUpdates,
                    setIsModalOpen: updater.setIsModalOpen,
                  }}
                />
              )}
            </div>
        ...
        <nav className="bottom-dock">
          ...
          <UpdateWidget
            status={updater.status}
            isOnline={updater.isOnline}
            versionLabel={updater.downloadLabel}
            onOpen={() => updater.setIsModalOpen(true)}
          />

          {updater.hasUpdate || updater.status === "offline" || !updater.isOnline ? (
            <span className="dock-separator" />
          ) : null}
      ...
      <AndroidFolderPicker
        ...
        onPick={({ serial, path, deviceName }) => {
          setPickerOpen(false);
          setAndroidFolder(serial, path, deviceName);
        }}
      />

      <UpdateModal
        open={updater.isModalOpen}
        status={updater.status}
        error={updater.error}
        info={updater.updateInfo}
        progress={updater.progress}
        isOnline={updater.isOnline}
        onClose={() => updater.setIsModalOpen(false)}
        onCheck={() => void updater.checkForUpdates(false)}
        onDownload={() => void updater.downloadAndInstall()}
        onRestart={() => void updater.restartApp()}
      />
